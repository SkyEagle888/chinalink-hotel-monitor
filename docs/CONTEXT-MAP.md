...
> Format: `.md` | Last updated: 2026-07-22
...
|---|---|---|---|
| `scrape_and_notify.py` | Entry point: scrape → hash → prefilter (region+keywords+expired) → fetch_detail_pages → **Discord (v1.3.1: skip if empty / v1.3.2: skip if page unchanged)** | 2026-07-22 | ✅ 72 unit tests |
...
| `last_hash.txt` | SHA-256 state for change detection (in-repo) | 2026-07-22 | ✅ 64 hex chars |
| `last_promos.json` | Full promo list for T9.6.3 diff tracking (in-repo) | 2026-07-22 | ✅ auto-managed |
| `README.md` | User-facing setup + behaviour docs | 2026-07-22 | ✅ |
| `AGENTS.md` | Project workflow rules + AI directives | 2026-06-06 | ✅ |
| `tests/test_t9.py` | 72 unittest cases (v1.3.2 regression suite: +6 TestNoChangeSkip) | 2026-07-22 | ✅ 72/72 pass |
...
| `docs/SCOPE.md` | Upstream requirements baseline (§1–§9 immutable, §10 v1.2, §11 v1.3 additive, §7 v1.3.1 跳過「無保留」 + v1.3.2 跳過「無變更」) | 2026-07-22 | ✅ |
| `docs/PLAN.md` | Implementation plan (T1–T12 all `[x]`) | 2026-07-22 | ✅ |
| `docs/ARCHITECTURE.md` | System topology + data model + business rules (v1.3.2 no-show tolerance updated) | 2026-07-22 | ✅ |
| `docs/CHANGE-LOG.md` | Session summaries (rolling 14d, 15KB trim threshold) | 2026-07-22 | ✅ |
| `docs/DB-SCHEMA.md` | N/A marker (no database in project) | 2026-06-06 | ✅ |
| `docs/CONTEXT-MAP.md` | This file | 2026-07-22 | ✅ |
...
- [x] `scrape_and_notify.py:847-982` `main` — v1.3.2 orchestration (**v1.3.2 新增**: 步驟 2 `current_hash == load_last_hash() → skip Discord + emit run.no_change`；**v1.3.1**: 步驟 5 `if not filtered: skip Discord` 邏輯)
- [x] `scrape_and_notify.py:112-130` `_log_event` + `_STRUCT_LOGGER` — T9.6.2 JSON Lines logger (unchanged)
- [x] `tests/test_t9.py:700-836` `TestNoChangeSkip` — T12.2.1 新增 6 案例（v1.3.2 跳過 Discord 行為 — 頁面無變更）
- [x] `tests/test_t9.py:838-967` `TestNoPromotionsSkip` — T11.3.1 新增 6 案例（v1.3.1 跳過 Discord 行為 — 頁面有更新但無保留卡片）

## Validation Status

- ✅ `requirements.txt` — 3 packages, compatible with Python 3.12
- ✅ `scrape_and_notify.py` — `py_compile` + **72 unit tests** (v1.3.2 refactor; +6 TestNoChangeSkip, T11.3.1 +6 TestNoPromotionsSkip)
- ✅ `.github/workflows/hotel-monitor.yml` — valid YAML, 3 jobs with scoped permissions (T11.4 + T12.3: 無需變更)
...
- ✅ **T12.1–T12.3** implemented (skip Discord when `current_hash == load_last_hash()`; `run.no_change` event enhanced with `total_promos`; 6 new `TestNoChangeSkip` cases)
