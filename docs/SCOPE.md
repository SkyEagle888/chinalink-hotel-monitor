# Requirements.md — 環島中港通酒店套票監察系統

> **目標網址：** https://www.tilchinalink.com/hotel_packages.php?lang=tc  
> **監察範圍：** 酒店套票（酒店住宿 + 車費 + 自助餐/早餐）— 僅 深圳 / 廣州 / 中山 / 珠海  
> **技術棧：** GitHub Actions + Python + Discord Webhook  
> **版本：** 1.3 | 17 June 2026  
> **語言：** 繁體中文輸出

> 📌 **v1.3.2 變更（§7）**：「頁面無變更時」改為靜默（不再發 Discord 通知）；詳見 §7「頁面無變更時」分節。與 v1.3.1「無保留卡片時靜默」對稱 — Discord 頻道僅承載「實質新套票」訊號。
>
> 📌 **v1.3.1 變更（§7）**：「無酒店套票時」改為靜默（不再發 Discord 通知）；詳見 §7「無酒店套票時」分節。
>
> 📌 **v1.3 重大變更（§11）**：目標網址由 `promotions.php` 切換至 `hotel_packages.php?lang=tc`，並加入地區白名單過濾；不再調用 LLM（依網站策展信任）。詳見 §11。

---

## 1. 背景及目標

環島中港通（Trans-Island Chinalink）是一間跨境客運公司，提供香港、澳門及中國大陸之間的客車及豪華轎車服務。其優惠頁面將酒店套票、演唱會、主題公園、車費折扣及充值獎賞等多種不同性質的優惠混合在同一列表中發布。

**業務需求：** 每日自動監察此頁面，篩選出符合條件的酒店套票（酒店住宿 + 車費 + 自助餐或早餐），以繁體中文格式發送至指定 Discord 頻道，時間為每日早上 09:00（HKT）。

---

## 2. 頁面分析

### 2.1 網站資料

| 屬性 | 詳情 |
|---|---|
| **網址** | https://www.tilchinalink.com/hotel_packages.php?lang=tc |
| **公司** | 環島中港通 / Trans-Island Chinalink |
| **預設語言** | 繁體中文（預設）；英文版本可透過 `?lang=en` 取得 |
| **抓取版本** | 繁體中文版本（預設 URL，不加 `?lang=en`） |
| **頁面渲染** | 伺服器端 PHP — 靜態 HTML 輸出 |
| **需要 JS 渲染** | ❌ 否 — 普通 HTTP `requests` 即可 |
| **抓取難度** | 🟢 低 |
| **分頁** | `?page=2`、`?page=3` …最多約 6 頁 |
| **個別優惠 URL** | `?id=147`（遞增整數 ID） |
| **估計每頁 Token 數** | ~3,000–6,000 tokens |
| **更新頻率** | 不定期 — 有新優惠時才更新 |

### 2.2 優惠類別分析

經分析頁面內容（2026年4月27日），同一列表中存在以下優惠類別：

| 類別 | 實際例子 | 是否納入 |
|---|---|---|
| **A — 酒店套票**（住宿 + 車費 + 自助餐/早餐） | 酒店 + 交通 + 餐飲套票 | ✅ 納入 |
| B — 演唱會 / 表演 | 郭富城、黎明演唱會；澳門2049 | ❌ 排除 |
| C — 主題公園 / 景點 | 深圳華發雪 Bonski、長隆 | ❌ 排除 |
| D — 純車費折扣 | 買去程送回程（無住宿） | ❌ 排除 |
| E — 充值 / 現金獎賞 | WeChat Pay / 支付寶充值優惠 | ❌ 排除 |
| F — 新路線開通優惠 | 廣州西、福永、順德半價優惠 | ❌ 排除 |
| G — 售票中心折扣券 | 北角 $10 即減 | ❌ 排除 |

> ⚠️ **重要提示：** 部分優惠將酒店名稱列為**上落車地點**（例如：花山希爾頓歡朋酒店為新路線站點、佳和花園酒店為上車點）。這些屬於**純車費優惠**，必須排除 — 優惠中出現酒店名稱並不代表屬於酒店套票。

### 2.3 抓取注意事項

- 各優惠之間無語義化 HTML 容器（如 `<article>`、`<section>`）— 以純文字段落及 `...` 分隔
- 無 JSON-LD / 結構化 Schema 標記
- 已過期優惠仍保留在頁面上，並無任何「已過期」標示 — 日期驗證至關重要
- 個別優惠 URL 格式：`https://www.tilchinalink.com/promotions.php?id=XXX`（`hotel_packages.php` 之卡片連結）
- 卡片結構：`<a class="package-wrapper" href="…?id=XXX">` 內含 `<h3 class="package-card-title">`、`📍 <span class="package-location">城市</span>`、`<span class="package-price">$XXX</span>`
- 抓取時須從頁面鏈接中提取各優惠的 `id` 參數，以便在通知中提供直接連結
- 詳情頁（`promotions.php?id=XXX`）含 `<div class="faintivory-background">`，內有發布日期 `發布日期: YYYY-MM-DD` 與套票說明

---

## 3. 範圍定義

### 3.1 合資格酒店套票 — 納入條件

只有同時符合以下**全部條件**的優惠，才會被視為合資格酒店套票：

| # | 條件 | 備注 |
|---|---|---|
| 1 | 包含酒店住宿（1晚或以上） | 必須為實際入住，而非僅於酒店大堂上落車 |
| 2 | 包含巴士／客車／豪華轎車交通（任何方向） | 可為單程或來回 |
| 3 | 包含自助餐**或**早餐（至少一項） | 必須包含餐飲項目 |
| 4 | 以套票形式（合併定價）出售 | 非單獨購買 |

### 3.2 自動排除條件

無論優惠標題或內容如何提及酒店，只要優惠主要屬於以下類型，均予排除：

- 演唱會、表演或音樂節套票
- 主題公園、雪場或遊樂場套票
- 觀光遊覽或導遊套票
- 純車費折扣（酒店僅為上落車地點）
- 充值積分、現金回贈或優惠券
- 新路線開通優惠（酒店僅為登車站點）

### 3.3 過期處理

- 將每個優惠的有效期與**執行日期**（已注入提示詞）進行比對
- 已過期優惠須靜默略過，不得出現在輸出內容中
- 沒有列明結束日期的優惠，視為**仍然有效**

---

## 4. 功能需求

### FR-1 — 網頁抓取

| ID | 需求 |
|---|---|
| FR-1.1 | 以 HTTP GET 方式抓取 `https://www.tilchinalink.com/hotel_packages.php?lang=tc`（繁體中文版本） |
| FR-1.2 | 不分頁（v1.3 觀察：頁面無 `?page=` 鏈接；如未來出現分頁，預留 `MAX_PAGES` 環境變量） |
| FR-1.3 | 若整頁優惠均早於 6 個月前發布，則停止翻頁 |
| FR-1.4 | 在 HTTP 請求標頭中設置有效的瀏覽器 `User-Agent` |
| FR-1.5 | 設置 30 秒請求超時；遇 `ConnectionError` 或 HTTP 5xx 時重試一次 |
| FR-1.6 | 移除 `<nav>`、`<footer>`、`<script>`、`<style>`、`<img>` 標籤後再提取文字 |
| FR-1.7 | **從頁面連結中提取個別優惠的 URL**（格式：`?id=XXX`），並與對應優惠文字配對 |
| FR-1.8 | 如主要內容選擇器失效，則回退至完整 `<body>` 文字 |

### FR-2 — 變更偵測

| ID | 需求 |
|---|---|
| FR-2.1 | 計算所有已抓取頁面合併原始文字的 SHA-256 雜湊值 |
| FR-2.2 | 將目前雜湊值與儲存於倉庫的 `last_hash.txt` 進行比對 |
| FR-2.3 | 若雜湊值**相同**：發送簡短「無變更」Discord 通知並退出 — **不調用 LLM** |
| FR-2.4 | 若雜湊值**不同**：繼續進行 LLM 摘要，並在成功後更新 `last_hash.txt` |
| FR-2.5 | 每次執行後將更新的 `last_hash.txt` 提交回倉庫 |

### FR-3 — LLM 分類及摘要

| ID | 需求 |
|---|---|
| FR-3.1 | 使用推薦模型（見第 6 節）調用 OpenRouter API |
| FR-3.2 | 在系統提示詞中注入 `TODAY_DATE`，讓模型能驗證優惠有效期 |
| FR-3.3 | 提示詞指示模型分類並篩選 — 僅輸出符合第 3 節條件的酒店套票 |
| FR-3.4 | 提示詞明確列出排除類別（演唱會、主題公園、純車費優惠等） |
| FR-3.5 | 輸出格式包含：套票名稱、有效期、價格、餐飲詳情、住宿詳情、訂票渠道、備注及優惠 URL |
| FR-3.6 | **輸出格式不包含交通時間表及路線詳情**（用戶可自行查閱官方網站） |
| FR-3.7 | 輸出結尾加入統計行：`✅ 找到 N 個酒店套票 | 🚫 已排除 M 個非酒店優惠` |
| FR-3.8 | 若無合資格套票：輸出 `🔍 今日無酒店套票。已排除 M 個其他優惠。` |
| FR-3.9 | **所有輸出使用繁體中文** |
| FR-3.10 | 傳送至 LLM 的內容上限為 12,000 tokens |
| FR-3.11 | 實施模型備用鏈：主要 → 次要 → 第三選擇（見第 6 節） |

### FR-4 — Discord 通知

| ID | 需求 |
|---|---|
| FR-4.1 | 透過 Incoming Webhook URL 向 Discord 頻道發送摘要 |
| FR-4.2 | 根據結果類型使用不同訊息標題（找到套票 / **無套票時不發送（v1.3.1）** / 無變更 / 錯誤） |
| FR-4.3 | 每個套票項目須包含**官方網站的個別優惠直接連結** |
| FR-4.4 | 每條訊息頁腳包含完整優惠頁面連結 |
| FR-4.5 | 抓取失敗時：發送含失敗原因的錯誤警報 |
| FR-4.6 | LLM 失敗時：發送錯誤警報（不發送空白或不完整的輸出） |
| FR-4.7 | Discord 訊息總長度不得超過 2,000 字元（Discord 限制） |

### FR-5 — 排程及觸發

| ID | 需求 |
|---|---|
| FR-5.1 | 透過 GitHub Actions cron 每日自動執行：**01:00 UTC（09:00 HKT）** |
| FR-5.2 | 支援 `workflow_dispatch` 手動觸發，用於即時執行及測試 |
| FR-5.3 | 工作流程必須具備 `contents: write` 權限以提交 `last_hash.txt` |

### FR-6 — 程序化預篩選（最小化 LLM 用量）

> **目標：** 在調用 LLM 之前，以 Python 程序化移除已過期及明顯非酒店類型的優惠，從而減少 Token 消耗及 API 調用次數。

| ID | 需求 |
|---|---|
| FR-6.1 | 解析 HTML 結構（`<div class="faintivory-background">`），將每頁優惠拆分為獨立區塊（標題、日期、內容、URL） |
| FR-6.2 | 根據優惠內容中識別到的結束日期（中文日期格式），程序化排除已過期優惠 |
| FR-6.3 | 根據排除關鍵詞列表（演唱會、主題公園、充值優惠等），程序化排除明顯非酒店套票 |
| FR-6.4 | 發布日期超過 180 天且內容中無明確結束日期或結束日期已過的優惠，視為過期並排除 |
| FR-6.5 | 預篩選後若無剩餘優惠 → **跳過 LLM 調用**，直接發送「無酒店套票」Discord 通知 |
| FR-6.6 | 僅將預篩選後的優惠內容發送至 LLM，以減少 Token 用量 |
| FR-6.7 | 若 HTML 結構變更導致無法解析任何優惠 → 回退至發送完整頁面文字至 LLM（確保不漏報） |
| FR-6.8 | 預篩選統計（排除數量）須記錄於執行日誌中 |

#### FR-6.3 排除關鍵詞列表

| 類別 | 關鍵詞 |
|---|---|
| 演唱會 / 表演 | 演唱會、音樂會、音樂節 |
| 主題公園 / 景點 | 主題公園、雪場、遊樂場 |
| 純車費優惠 | 買去程送回程、車票半價 |
| 充值 / 金融 | 充值優惠、充值獎賞、消費券、現金回贈 |
| 新路線 / 新站點 | 新增站點、站點限定 |
| 其他服務 | 行李托運 |

---

## 5. 非功能需求

| 屬性 | 需求 |
|---|---|
| **費用** | $0 — GitHub Actions 免費版（每月 2,000 分鐘）；OpenRouter 免費模型 |
| **執行時間** | 完整流程在 3 分鐘內完成 |
| **精確度** | 誤判（非酒店優惠出現在輸出中）不可接受；漏判為可接受的邊緣情況 |
| **可靠性** | 每次執行均必須發送 Discord 通知 — 摘要、無變更提示或錯誤警報 |
| **安全性** | `OPENROUTER_API_KEY` 及 `DISCORD_WEBHOOK_URL` 僅儲存於 GitHub Secrets |
| **冪等性** | 同一天執行兩次，頁面無變化時產生相同 Discord 輸出 |
| **Token 效率** | 頁面無變更時 0 次 LLM 調用；預篩選後若無候選優惠亦 0 次 LLM 調用；有候選時僅發送篩選後內容 |
| **可維護性** | 提示詞、模型 ID、URL 及篩選條件以常數形式定義；無硬編碼業務邏輯 |

---

## 6. OpenRouter 免費模型推薦

> ⚠️ **香港帳戶限制：** 由於 OpenRouter.ai 帳戶位於香港，以下提供商的模型**不可使用**：
> - ❌ **Anthropic**（Claude 系列）
> - ❌ **OpenAI**（GPT 系列）
> - ❌ **Google**（Gemini / Gemma 系列）
>
> 以下推薦均為非上述提供商的免費模型。

### 6.1 模型優先級列表

| 優先級 | 模型 ID | 提供商 | 上下文 | 繁體中文 | 狀態 |
|---|---|---|---|---|---|
| **1 — 主要** | `qwen/qwen3-next-80b:free` | Alibaba（阿里巴巴） | 262K | ⭐⭐⭐⭐⭐ 原生 | 穩定免費 |
| **2 — 備用** | `z-ai/glm-4.5-air:free` | Z.AI（智譜 AI） | 131K | ⭐⭐⭐⭐⭐ 原生 | 穩定免費 |
| **3 — 備用** | `meta-llama/llama-3.3-70b-instruct:free` | Meta | 131K | ⭐⭐⭐ 良好 | 穩定免費 |

### 6.2 選擇理由

**主要：`qwen/qwen3-next-80b:free`（Qwen3-Next 80B）**
- 阿里巴巴原生中文大模型，對繁體中文（粵港澳用語）理解能力最佳
- 針對 RAG 及 Agentic 工作流程優化，分類準確性高
- 262K token 上下文 — 單次 API 調用即可處理所有分頁優惠內容
- 完全免費，非 Google / OpenAI / Anthropic

**次要：`z-ai/glm-4.5-air:free`（GLM-4.5-Air）**
- 智譜 AI 原生中文模型，繁體中文輸出質量優秀
- 混合推理模式（思考模式 / 非思考模式），可提升日期驗證準確性
- 131K token 上下文，MoE 架構效率高

**第三選擇：`meta-llama/llama-3.3-70b-instruct:free`（Llama 3.3 70B）**
- GPT-4 同等整體性能
- 中文理解能力遜於 Qwen / GLM，但作為最後備用方案仍可接受
- 131K token 上下文

> ⚠️ **DeepSeek 注意事項：** 雖然 DeepSeek 模型中文能力優秀且免費，但其伺服器位於中國大陸，所有提示詞均會用於模型訓練且無法退出，存在數據私隱風險。本方案不建議使用。

### 6.3 頻率限制說明

OpenRouter 免費模型限制：每分鐘 20 個請求，每日 200 個請求。
本流程每日最多使用 **0–1 次 LLM 調用**（頁面無變更時為 0 次），遠低於限制。

---

## 7. Discord 訊息格式（繁體中文）

### 找到酒店套票時：
```
🏨 環島中港通 酒店套票快訊 | 27/04/2026

🏨 **【套票名稱】**
📅 有效期：【日期】
💰 價格：【每位價格或「請查閱官網」】
🍽️ 餐飲：【自助餐 / 早餐詳情】
🛏️ 住宿：【晚數 + 房型】
📲 訂票：【渠道：微信小程式 / 支付寶 / 售票中心】
📝 備注：【主要限制或亮點，最多一句】
🔗 優惠詳情：https://www.tilchinalink.com/promotions.php?id=XXX

✅ 找到 1 個酒店套票 | 🚫 已排除 6 個非酒店優惠
━━━━━━━━━━━━━━━━━━━━━━
🔗 查閱所有優惠 → https://www.tilchinalink.com/promotions.php
```

### 無酒店套票時：
**v1.3.1 起跳過 Discord 通知** — 頁面有更新但無任何保留卡片時，腳本不發送任何
Discord 訊息（避免噪音），僅：
- 寫入結構化日誌 `run.no_promotions` 事件（含 `total_promos` / `region_excluded` / `other_excluded`）
- 更新 `last_hash.txt` + `last_promos.json`（避免下次重複處理同一空結果）
- 寫入 `run.end` 事件並標記 `discord_sent: false`
- 監察 job 仍正常結束（exit 0）— GitHub Action 不會因無通知而失敗

> ℹ️ **行為變更（v1.3.1）**：v1.3 仍會發送 `🔍 環島中港通 酒店套票 | 27/04/2026` 一行「無套票」通知。v1.3.1 改為完全靜默 — 反映用戶實際關注點為「是否有新套票可買」，而非「今日無套票」的日常確認。

### 頁面無變更時：
**v1.3.2 起跳過 Discord 通知** — 頁面 SHA-256 與上次相同時（無論頁面上有多少卡片），
腳本不發送任何 Discord 訊息，僅：
- 寫入結構化日誌 `run.no_change` 事件（含 `pages` / `total_promos`）
- 直接 `return` — 不觸發 `run.end`，不更新狀態檔（因為雜湊值未變更）

> ℹ️ **行為變更（v1.3.2）**：v1.3 / v1.3.1 仍會發送 `ℹ️ 環島中港通 酒店套票 — 今日頁面無更新（…）` 一行確認通知。v1.3.2 改為完全靜默 — 反映用戶實際關注點為「是否有新套票可買」，而非「今日無更新」的日常確認。

> ℹ️ **與 v1.3.1 的對比**：
> - v1.3.1：頁面**有更新**但 prefilter 全部排除 → 靜默（emit `run.no_promotions`）
> - v1.3.2：頁面**無更新** → 靜默（emit `run.no_change`）
> - 兩者設計理念一致：**Discord 頻道僅承載「實質新套票」訊號**，日常確認訊息交由結構化日誌即可追溯。

### 發生錯誤時：
```
⚠️ 優惠監察機器人錯誤 | 27/04/2026
【抓取失敗 / LLM 失敗】：【錯誤信息】— 請手動檢查。
```

---

## 8. 所需 Secrets

| Secret 名稱 | 說明 |
|---|---|
| `OPENROUTER_API_KEY` | ⚠️ **v1.3+ 不再需要** — v1.2 LLM pipeline 已移除；保留以備未來重新啟用（刪除亦無害） |
| `DISCORD_WEBHOOK_URL` | 來自 Discord 頻道設置的 Incoming Webhook URL（**v1.3+ 唯一必需的 secret**） |

## 9. 所需 Variables

以下參數透過 **GitHub Actions → Variables** 設定：

| Variable 名稱 | 預設值 | 說明 |
|---|---|---|
| `OPENROUTER_BASE_URL` | `https://openrouter.ai/api/v1` | ⚠️ **v1.3+ 不再使用** — LLM pipeline 已移除；保留供 v1.4+ |
| `OPENROUTER_MODEL_PRIMARY` | `qwen/qwen3-next-80b:free` | ⚠️ **v1.3+ 不再使用** — 主要模型（Alibaba Qwen） |
| `OPENROUTER_MODEL_SECONDARY` | `z-ai/glm-4.5-air:free` | ⚠️ **v1.3+ 不再使用** — 備用模型（智譜 AI GLM） |
| `OPENROUTER_MODEL_TERTIARY` | `meta-llama/llama-3.3-70b-instruct:free` | ⚠️ **v1.3+ 不再使用** — 第三備用模型（Meta Llama） |

> ℹ️ **v1.3 起**：上述 OPENROUTER_* 全部為 **legacy / no-op**。Pipeline 完全 deterministic（trust site labels + 4-region whitelist + keyword exclusion + 180-day expiry），每月成本 **$0**。如要徹底清理 GitHub repo，可從 Secrets / Variables 頁面刪除 — 對執行無副作用。

---

## 10. 增強提案（v1.2 — 已完成 2026-06-06）

> **狀態**：全部 19 個項目已實作並通過驗證。實作記錄見 `docs/PLAN.md` T9 + `docs/CHANGE-LOG.md`。  
> **未列入（低 ROI，v1.3 再評估）**：EFF-5（並行模型 first-wins）、EFF-6（hash 快取）、ROB-5（週報）。
> **範圍**：附加層，不取代 §4 FR-1 至 FR-6 既有需求。  
> **實作任務**：見 `docs/PLAN.md` T9。

### 10.1 已知缺陷修正（🔴 必修）

| ID | 來源 | 問題 | 修正方向 |
|---|---|---|---|
| BUG-1 | FR-1.3 | 早停邏輯未實作（`scrape_and_notify.py:247`），永遠抓滿 3 頁 | 抓取後比對頁面最新發布日，若全部 >180 天則 `break` |
| BUG-2 | FR-6.4 | `PROMO_STALE_DAYS=60` 與本節規定 180 天不一致 | 統一為 180 天並透過環境變量可調 |
| BUG-3 | FR-2.1 | 雜湊前未歸一化（廣告/計數器/隨機 ID 觸發重跑） | strip 動態元素後再 hash |

### 10.2 效率增強（EFF）

| ID | 需求 | 預期效益 |
|---|---|---|
| EFF-1 | `ThreadPoolExecutor` 並行抓取 3 頁 | 抓取時間 ~3s → ~1.5s |
| EFF-2 | 雜湊前內容歸一化（去廣告、計數器、隨機 id） | 降低假陽性觸發 LLM |
| EFF-3 | 早停：當前頁所有優惠 >180 天則停止翻頁 | 實作 BUG-1 + 節省 token |
| EFF-4 | 候選 ≥3 時啟發式第二輪篩選（雙關鍵字命中） | 多數日子維持 0–1 API 調用 |
| EFF-5 | 模型並行 + 首響應採納（fallback 改 first-wins） | 主要模型慢時不延遲 |
| EFF-6 | LLM 結果按 `hash(內容)` 快取 | 重跑時免費 |

### 10.3 準確度增強（ACC）

| ID | 需求 | 預期效益 |
|---|---|---|
| ACC-1 | LLM 切換至 JSON schema 結構化輸出 | 消除 `count_hotel_packages` 脆弱 regex（`scrape_and_notify.py:427`） |
| ACC-2 | 擴展 `extract_end_dates` regex 覆蓋「即日起」+ 無年 + slash 格式 | 減少漏抓「無年結束日」優惠 |
| ACC-3 | 反向白名單：「住宿」「入住」「房間」必須出現才算酒店類 | 抑制「酒店名僅為站點」的誤判 |
| ACC-4 | system prompt 注入 2 合格 + 2 誤判 few-shot 範例 | 小模型分類精度提升 |
| ACC-5 | 輸出驗證層：解析後確認 `?id=XXX` URL 存在，否則重試 | 捕捉 LLM 偶發遺漏 |
| ACC-6 | 評估集（黃金 20 條）+ GitHub Action 評估 job | 可量化改進成效 |
| ACC-7 | Self-consistency：同 prompt 跑 2 次取交集 | 過濾隨機錯誤 |

### 10.4 穩健性 / 可觀測性增強（ROB）

| ID | 需求 |
|---|---|
| ROB-1 | Discord Webhook 重試（exponential backoff, 3 次） |
| ROB-2 | 結構化 JSON 日誌（含 `run_id`、模型、token 用量） |
| ROB-3 | 逐頁雜湊 + 變更差異高亮（新增/移除的優惠 id） |
| ROB-4 | 限縮 workflow `contents: write` 權限至 `last_hash.txt` 路徑 |
| ROB-5 | 週報訊息：週日發 7 天命中率摘要至 Discord |
| ROB-6 | `DRY_RUN=true` 環境變量支持（測試用，不發 Discord） |

### 10.5 推薦實作順序

| # | ID | 理由 |
|---|---|---|
| 1 | BUG-1 / BUG-2 / BUG-3 | 修正缺陷，消除已知漂移 |
| 2 | ACC-1 + ACC-5 | 結構化輸出 + 驗證層，從根上改善解析 |
| 3 | EFF-2 + EFF-3 | 雜湊歸一化 + 早停，最大 ROI |
| 4 | ACC-6 | 評估集 + CI，建立可量化的改進基礎 |
| 5 | ACC-2 / ACC-3 | 日期 regex 擴展 + 反向白名單 |
| 6 | ROB-1 / ROB-2 | 重試 + 結構化日誌 |
| 7 | EFF-4 / EFF-5 / EFF-6 / ROB-3..6 | 視 ROI 決定 |

### 10.6 對既有章節的影響
- §3 範圍定義不變（合資格/排除條件維持）
- §4 FR-1 至 FR-6 維持 — 上述增強為附加層
- §5 NFR 增加「可評估性」（ACC-6 評估集）維度
- §7 Discord 訊息格式不變（除非 ACC-1 改 JSON 內部表示，不影響輸出）

---

## 11. v1.3 增強提案 — 目標網址切換 + 地區白名單（已完成 2026-06-17）

> **狀態**：已實作並通過驗證（60 單元測試 + 20/20 黃金集）。實作記錄見 `docs/PLAN.md` T10 + `docs/CHANGE-LOG.md`。  
> **動機**：v1.2 持續運行期間觀察到：(a) 網站新增 `hotel_packages.php` 頁面，網站自身已策展為「酒店套票」專屬頁；(b) 用戶希望僅監察 深圳/廣州/中山/珠海 4 個地區。  
> **範圍**：本節取代 v1.1 的 §2.1（網址）、§4 FR-1.1（抓取目標）與 §7（Discord 訊息按地區分組）。其他章節（§3 合資格條件、§5 NFR、§6 模型推薦）仍以 v1.2 為準 — v1.3 不再使用 OpenRouter LLM，§6 的模型備用鏈自 v1.3 起標記為「保留供未來 v1.4 擴展」。

### 11.1 目標網址變更（v1.3）

| 屬性 | v1.2 | v1.3 |
|---|---|---|
| **目標網址** | `promotions.php` | **`hotel_packages.php?lang=tc`** |
| **頁面性質** | 混合優惠（演唱會/主題公園/純車費/酒店套票/充值優惠） | **酒店套票專屬**（網站策展） |
| **抓取難度** | 🟢 低（PHP 服務器渲染） | 🟢 低 |
| **分頁** | `?page=2`/`?page=3` | **不分頁**（`MAX_PAGES=1`） |
| **LLM 角色** | 必要（從混合內容中分類） | **不再使用**（頁面已策展） |
| **每張卡片連結** | `promotions.php?id=XXX` | `promotions.php?id=XXX`（相同） |

### 11.2 地區白名單（v1.3 新增）

依用戶指定，僅監察以下 4 個地區之卡片：

| 地區 | 優先級 | 卡片數（2026-06-17 觀察） |
|---|---|---|
| 深圳 | 1 | 8 |
| 廣州 | 2 | 2（皆為滑雪套票，被 v1.3 §11.3 排除） |
| 中山 | 3 | 2 |
| 珠海 | 4 | 1 |
| **最終每日保留** | — | **11** |

- `INCLUDE_REGIONS = ["深圳", "廣州", "中山", "珠海"]`（常數，位於 `scrape_and_notify.py`）
- `region_allowed(promo)` 在 `prefilter()` 之第一步執行
- Discord 訊息 header 動態顯示「**深圳 / 中山 / 珠海 酒店套票快訊**」（廣州 0 張時省略）

### 11.3 排除關鍵詞精簡（v1.3）

v1.2 的 `EXCLUDE_KEYWORDS`（演唱會/主題公園/充值優惠等共 14 條）在 v1.3 大幅精簡，**僅保留與 hotel_packages.php 相關之排除**：

| 保留 | 排除原因 |
|---|---|
| `滑雪` | 滑雪套票（如廣州融創花間堂+滑雪、花都融創施柏閣+滑雪）— 屬「雪場套票」，依 v1.2 §3.2 排除 |
| `雪場` | 同上 |
| `雪場套票` | 同上 |

v1.2 的其他排除（演唱會、主題公園、純車費、充值優惠、行李托運等）已無需 — `hotel_packages.php` 頁面只列酒店套票，無混合內容。

### 11.4 Detail-page 抓取（v1.3 新增）

每張保留卡片對應一個詳情頁（`promotions.php?id=XXX`）。v1.3 新增 `fetch_detail_pages()` 並行抓取所有 unique 詳情頁，提取：

- `發布日期: YYYY-MM-DD`（用於 `is_expired()` 判斷）
- `nights`（如「1晚」「2晚」）
- `dining`（如「自助餐」「早餐」）
- `transport`（如「直通巴士」）
- `room_type`（如「標準房」「大床房」）

抓取細節：
- `ThreadPoolExecutor(max_workers=DETAIL_FETCH_WORKERS=4)`（env 可調）
- `DETAIL_FETCH_TIMEOUT=15s`（env 可調）
- 失敗時 fallback 為 card-only 顯示（`請查閱官網` 標記）
- 11 張卡片 / 4 unique URL → 預計 <2 秒（受 1.5s politeness 影響）

### 11.5 Discord 訊息格式變更（v1.3）

v1.2 訊息為單一列表；v1.3 改為按地區分組：

```
🏨 環島中港通 深圳 / 中山 / 珠海 酒店套票快訊 | 17/06/2026

━━ 深圳（8 個套票）━━

🏨 **寶安登喜路國際大酒店套票**
💰 價格：HK$365 起/位
🛏️ 住宿：1 晚，標準房
🍽️ 餐飲：自助早餐
🚍 交通：直通巴士
📅 發布：2026-05-12
🔗 優惠詳情：https://www.tilchinalink.com/promotions.php?id=85&lang=tc

🏨 **深圳同泰萬怡酒店套票**
…

━━ 中山（2 個套票）━━

🏨 **中山石岐步行街興中廣場希岸Deluxe酒店套票**
…

━━ 珠海（1 個套票）━━

🏨 **珠海金灣華發萬豪酒店套票**
…

✅ 共 11 個酒店套票 | 🚫 已排除 11 個其他優惠
━━━━━━━━━━━━━━━━━━━━━━
📊 掃描：1 頁 | 卡片：22 | 地區排除：7 | 其他排除：4 | 最終保留：11
━━━━━━━━━━━━━━━━━━━━━━
🔗 查閱所有套票 → https://www.tilchinalink.com/hotel_packages.php?lang=tc
```

「無酒店套票」訊息（v1.3 — **v1.3.1 起跳過 Discord 通知**）：

> ⚠️ **v1.3.1 變更**：v1.3 仍會發送 `🔍 環島中港通 酒店套票 | 17/06/2026` 一行「無套票」通知。v1.3.1 起改為**完全靜默** — 詳見 §7「無酒店套票時」段。

### 11.6 對既有章節的影響（v1.3）

| 章節 | v1.3 變更 |
|---|---|
| §2.1（網站資料） | 目標網址替換；分頁移除；卡片結構描述新增 |
| §3.1（合資格） | **不變**（仍要求住宿+車費+餐飲） |
| §3.2（自動排除） | 新增 §11.3 精簡清單；滑雪/雪場保留 |
| §4 FR-1.1 | URL 替換 |
| §4 FR-1.2 | 分頁移除（保留 `MAX_PAGES` 變量供未來） |
| §6（OpenRouter 模型） | **不再使用** — 標記為「保留供 v1.4」 |
| §7（Discord 格式） | 按地區分組；新增 detail-page 豐富欄位 |
| §9（所需 Variables） | 移除 `OPENROUTER_*`（仍可保留以備未來）；新增 `DETAIL_FETCH_TIMEOUT` / `DETAIL_FETCH_WORKERS` |

### 11.7 未列入 v1.3（低 ROI，v1.4 再評估）

- 地區白名單的 env 可調（目前寫死 — 暫不開放）
- 跨日併發去重（多張卡同 id 不會重複通知 — 因 Discord 訊息按卡片粒度而非 id 粒度）
- 卡片排序改為價格升序（目前維持卡片原始順序）
- 多語系支援（`?lang=en` 版 — 暫無需求）

---

*版本 1.3 | 17 June 2026 | 環島中港通酒店套票監察系統 | Henry Fok / Legato Technologies Limited*
